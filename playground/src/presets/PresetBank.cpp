...
void cleanupPresetColorTagForOverwriteSave(Preset *presetToOverwrite, Preset *src) {
  if(src->getAttribute("color", "none") == "none")
    src->setAttribute("color", presetToOverwrite->getAttribute("color", "none"));
}

void PresetBank::undoableOverwritePreset(UNDO::Scope::tTransactionPtr transaction, Preset *presetToOverwrite, Preset *src)
{
  cleanupPresetColorTagForOverwriteSave(presetToOverwrite, src);
