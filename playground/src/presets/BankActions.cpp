    ...
    , m_presetManager(presetManager)
{
  ...
  addAction("delete-preset", [&](shared_ptr<NetworkRequest> request) mutable {
    ...
    if(tBankPtr srcBank = m_presetManager.findBankWithPreset(presetUUID))
    {
      ...
      {
        UNDO::Scope::tTransactionScopePtr scope
            = m_presetManager.getUndoScope().startTransaction(preset->getUndoTransactionTitle("Delete"));
        UNDO::Scope::tTransactionPtr transaction = scope->getTransaction();
  ...
  addAction("load-preset", [&](shared_ptr<NetworkRequest> request) mutable {
    ...
    if(tBankPtr bank = m_presetManager.findBankWithPreset(uuid))
    {
      ...
      {
        UNDO::Scope::tTransactionScopePtr scope
            = m_presetManager.getUndoScope().startTransaction(preset->getUndoTransactionTitle("Load"));
        UNDO::Scope::tTransactionPtr transaction = scope->getTransaction();
  ...
  addAction("set-position", [&](shared_ptr<NetworkRequest> request) mutable {
    ...
    if(tBankPtr bank = m_presetManager.findBank(uuid))
    {
      ...
      {
        UNDO::Scope::tTransactionScopePtr scope
            = presetManager.getUndoScope().startTransaction("Move preset bank '%0'", bank->getName(true));
        UNDO::Scope::tTransactionPtr transaction = scope->getTransaction();
