...
{
  LPC_USBB->DEVICEADDR = USBDEV_ADDR(adr);
  LPC_USBB->DEVICEADDR |= USBDEV_ADDR_AD;
}
...
static void USB_Reset(void)
{
  ...
  /* disable all EPs */
  LPC_USBB->ENDPTCTRL0 &= ~(EPCTRL_RXE | EPCTRL_TXE);
  LPC_USBB->ENDPTCTRL1 &= ~(EPCTRL_RXE | EPCTRL_TXE);
  LPC_USBB->ENDPTCTRL2 &= ~(EPCTRL_RXE | EPCTRL_TXE);
  LPC_USBB->ENDPTCTRL3 &= ~(EPCTRL_RXE | EPCTRL_TXE);

  /* Clear all pending interrupts */
  LPC_USBB->ENDPTNAK       = 0xFFFFFFFF;
  LPC_USBB->ENDPTNAKEN     = 0;
  LPC_USBB->USBSTS_D       = 0xFFFFFFFF;
  LPC_USBB->ENDPTSETUPSTAT = LPC_USBB->ENDPTSETUPSTAT;
  LPC_USBB->ENDPTCOMPLETE  = LPC_USBB->ENDPTCOMPLETE;
  while (LPC_USBB->ENDPTPRIME) /* Wait until all bits are 0 */
  {
  }
  LPC_USBB->ENDPTFLUSH = 0xFFFFFFFF;
  while (LPC_USBB->ENDPTFLUSH)
    ; /* Wait until all bits are 0 */

  /* Set the interrupt Threshold control interval to 0 */
  LPC_USBB->USBCMD_D &= ~0x00FF0000;
  ...
  /* init list address */
  LPC_USBB->ENDPOINTLISTADDR = (uint32_t) ep_QH;
  ...
  ep_QH[1].cap = QH_MAXP(USB_MAX_PACKET0)
      ...
      | QH_ZLT;
  /* enable EP0 */
  LPC_USBB->ENDPTCTRL0 = EPCTRL_RXE | EPCTRL_RXR | EPCTRL_TXE | EPCTRL_TXR;
  return;
}
...
static uint32_t USB_ReadSetupPkt(uint32_t EPNum, uint32_t *pData)
{
  ...
  setup_int = LPC_USBB->ENDPTSETUPSTAT;
  /* Clear the setup interrupt */
  LPC_USBB->ENDPTSETUPSTAT = setup_int;
  ...
  /* No setup are admitted on other endpoints than 0 */
  {
    ...
    } while (!(LPC_USBB->USBCMD_D & USBCMD_SUTW));

    /* setup in a setup - Clear the tripwire */
    LPC_USBB->USBCMD_D &= (~USBCMD_SUTW);
  }
  while ((setup_int = LPC_USBB->ENDPTSETUPSTAT) != 0)
  {
    /* Clear the setup interrupt */
    LPC_USBB->ENDPTSETUPSTAT = setup_int;
  }
...
void USBB_Core_Init(void)
{
  ...
  LPC_USBB->USBCMD_D = USBCMD_RST;
  /* wait for reset to complete */
  while (LPC_USBB->USBCMD_D & USBCMD_RST)
    ;

  /* Program the controller to be the USB device controller */
  LPC_USBB->USBMODE_D = USBMODE_CM_DEV
      | USBMODE_SDIS
      | USBMODE_SLOM;
  ...
  LPC_USBB->OTGSC = (1 << 3) | (1 << 0) /*| (1<<16)| (1<<24)| (1<<25)| (1<<26)| (1<<27)| (1<<28)| (1<<29)| (1<<30)*/;
#endif
...
{
  return (LPC_USBB->PORTSC1_D & (1 << 0)) && USB_Configuration;
}
...
void USBB_DataInStage(void)
{
  ...
  cnt = USBB_WriteEP(0x80, EP0Data.pData, cnt);
  EP0Data.pData += cnt;
  EP0Data.Count -= cnt;
}
...
void USBB_DataOutStage(void)
{
  ...
  cnt = USBB_ReadEP(0x00, EP0Data.pData);
  EP0Data.pData += cnt;
  EP0Data.Count -= cnt;
}
...
*******************************************************************************/
uint32_t USBB_ReqGetStatus(void)
{
  uint32_t n, m;
...
static uint32_t USB_SetTestMode(uint8_t mode)
{
  ...
  if ((mode > 0) && (mode < 8))
  {
    portsc = LPC_USBB->PORTSC1_D & ~(0xF << 16);

    LPC_USBB->PORTSC1_D = portsc | (mode << 16);
    return TRUE;
  }
  return (FALSE);
}
...
static void USB_SetStallEP(uint32_t EPNum)
{
  ...
  if (EPNum & 0x80)
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_TXS;
  }
  else
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_RXS;
  }
}
...
static void USB_ClrStallEP(uint32_t EPNum)
{
  ...
  if (EPNum & 0x80)
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] &= ~EPCTRL_TXS;
    /* reset data toggle */
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_TXR;
  }
  else
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] &= ~EPCTRL_RXS;
    /* reset data toggle */
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_RXR;
  }
}
...
/** @brief		Set/Clear Feature USB request
    ...
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqSetClrFeature(uint32_t sc)
{
  uint32_t n, m;
...
/** @brief		Set Address USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqSetAddress(void)
{
...
/** @brief		Get Descriptor USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqGetDescriptor(void)
{
  uint8_t *pD;
  uint32_t len, n;
...
static void USB_ConfigEP(USB_ENDPOINT_DESCRIPTOR *pEPD)
{
  ...
  ep_cfg = ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep];
  ...
  ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] = ep_cfg;
  return;
}
...
static void USB_EnableEP(uint32_t EPNum)
{
  ...
  if (EPNum & 0x80)
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_TXE;
#if USB_POLLING
    /* enable NAK interrupt for IN transfers - useful in polling mode only*/
    bitpos = USB_EP_BITPOS(EPNum);
    LPC_USBB->ENDPTNAKEN |= (1 << bitpos);
#endif
  }
  else
  {
    ((uint32_t *) &(LPC_USBB->ENDPTCTRL0))[lep] |= EPCTRL_RXE;
...
void USBB_ResetEP(uint32_t EPNum)
{
  ...
  /* flush EP buffers */
  LPC_USBB->ENDPTFLUSH = (1 << bit_pos);
  while (LPC_USBB->ENDPTFLUSH & (1 << bit_pos))
    ;
  /* reset data toggles */
  if (EPNum & 0x80)
  {
...
/** @brief		Get Configuration USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqGetConfiguration(void)
{
...
/** @brief		Set Configuration USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqSetConfiguration(void)
{
...
/** @brief		Get Interface USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqGetInterface(void)
{
...
/** @brief		Set Interface USB request
    @return		TRUE - success; FALSE - error
*******************************************************************************/
uint32_t USBB_ReqSetInterface(void)
{
  USB_COMMON_DESCRIPTOR *pD;
...
static void USB_EndPoint0(uint32_t event)
{
  switch (event)
  {
    ...
    case USB_EVT_OUT_NAK:
      ...
      {
        USBB_ReadReqEP(0x00, EP0Data.pData, EP0Data.Count);
      }
      else
      {
        /* might be zero length pkt */
        USBB_ReadReqEP(0x00, EP0Data.pData, 0);
      }
      break;
    case USB_EVT_OUT:
      if (SetupPacket.bmRequestType.BM.Dir == REQUEST_HOST_TO_DEVICE)
      {
        ...
        {                      /* still data to receive ? */
          USBB_DataOutStage(); /* receive data */
          if (EP0Data.Count == 0)
          { /* data complete ? */
            switch (SetupPacket.bmRequestType.BM.Type)
            {
      ...
      {
        USBB_StatusOutStage(); /* receive Acknowledge */
      }
...
#endif
{
  ...
  if (val)
  {
    ...
    LPC_USBB->ENDPTNAKEN |= 0x00010001;
  ...
  if (val)
  {
    ...
    if (val & (1 << 16))
    {
      ...
      LPC_USBB->ENDPTCOMPLETE = (1 << 16);
      USB_P_EP[0](USB_EVT_IN);
    }
    /* EP 1 - OUT */
    if (val & 2)
    {
      LPC_USBB->ENDPTCOMPLETE = 2;
  ...
  if (disr & USBSTS_NAKI)
  {
    val = LPC_USBB->ENDPTNAK;
    val &= LPC_USBB->ENDPTNAKEN;
...
*******************************************************************************/
void USBB_ProgDTD(uint32_t Edpt, uint32_t ptrBuff, uint32_t TsfSize)
{
  DTD_T *pDTD;
...
uint32_t USBB_WriteEP(uint32_t EPNum, uint8_t *pData, uint32_t cnt)
{
  ...
  USBB_ProgDTD(EPAdr(EPNum), (uint32_t) pData, cnt);
  /* prime the endpoint for transmit */
  LPC_USBB->ENDPTPRIME |= (1 << n);
...
uint32_t USBB_ReadReqEP(uint32_t EPNum, uint8_t *pData, uint32_t len)
{
  ...
  USBB_ProgDTD(num, (uint32_t) pData, len);
